...
const AGSHealth = (props) => {
    return (
        <div className="agsHealth">
            <h3 className="Title">
                {props.AgsHealth?.title}
                {/* HEALTH AND SAFETY */}
            </h3>
            <Container>
                <Row>
                    <Col sm={12} lg={5}>
                        <img alt={"#"} src={props?.AgsImg} className="healtImg" />
                    </Col>
                    <Col sm={12} lg={7}>
                        <p className="subtitle"
                            dangerouslySetInnerHTML={{
                                __html:
                                    props.AgsHealth?.description
                            }}
                        >
                            {/*  American Gulf School believes that the safety of every child is paramount and as such, places immense importance on the health, safety, and wellbeing of all its learners.
